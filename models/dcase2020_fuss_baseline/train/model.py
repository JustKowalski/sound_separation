...
#负对数MSE损失，SNR分母的负对数。todo loss_mse
...
def separate_waveforms(mixture_waveforms, hparams,is_graph):
 ...
 ######################################################
  # power = mixture_coeffs_input ** 2
  # sess = tf.Session()
  # power = sess.run(power)
  # for i in range(int(mixture_waveforms.shape[0])):
  #     temp = lib.feature.melspectrogram(S=power[i, 0])
  ...
  ####################################################
  mixture_coeffs_input = tf.transpose(mixture_coeffs_input, [0, 2, 3, 1])
  for i in range(mixture_coeffs_input.shape[0]):
      temp = tf.gather(mixture_coeffs_input, i)
      temp = spectrogram.random_time_stretch(temp)
      #temp = spectrogram.random_pitch_shift(temp)
      temp = tf.expand_dims(temp, 0)
      if i == 0:
          res = temp
      else:
          res = tf.concat([res, temp], axis=0)
  mixture_coeffs_input = res
  for i in range(mixture_coeffs_input.shape[0]):
      temp = tf.gather(mixture_coeffs_input, i)
      temp = spectrogram.random_pitch_shift(temp)
      temp = tf.expand_dims(temp, 0)
      if i == 0:
          ...
          res = tf.concat([res, temp], axis=0)
  mixture_coeffs_input = tf.transpose(mixture_coeffs_input, [0, 3, 1, 2])
  #####################################################
  mel = transformer.forward_mfcc(mixture_coeffs_input,is_graph)
  mixture_coeffs_input = tf.concat([mixture_coeffs_input, mel], axis=-1)
  #取幅度 todo BN与LN的区别，这里到底是BN还是LN?是BN，详见network.py line69
  mixture_coeffs_input = network.LayerNormalizationScalarParams(
      axis=[-3, -2, -1],
      name='layer_norm_on_mag').apply(mixture_coeffs_input)
  #倒数计数机制，最后一个是-1，倒数第二个是-2
  shaper.register_axes(mixture_coeffs_input, ['batch', 'mic', 'frame', 'bin'])
  shaper.register_axes(mixture_coeffs,['batch', 'mic', 'frame', 'spec_bin'])
  ...
  # zero_matrix = tf.zeros(shape=[3, 128, 256])
  # core_activations_out = tf.concat([core_activations,zero_matrix],axis=1)
  ...
  shaper.register_axes(mixture_coeffs, ['batch', 'mic', 'frame', 'spec_bin'])
  shaper.register_axes(
      activations, ['batch', 'frame', ('mic', 'source', 'spec_bin')])
    #spec_bins数值为未拼接mfcc特征的数值
  # ori
  ...
  activations = shaper.change(
      activations, ['batch', 'frame', ('mic', 'source', 'spec_bin')],
      ['batch', 'source', 'mic', 'frame', 'spec_bin'])
  #activations = tf.reshape(activations,[1,4,1,1250,257])
  mask = network.get_activation_fn('sigmoid')(activations)#(1,4,1,none,257)
  ...
  shaper.register_axes(mask_input, ['batch', '1', 'mic', 'frame', 'spec_bin'])
  separated_coeffs = mask * mask_input
  shaper.register_axes(
      separated_coeffs, ['batch', 'source', 'mic', 'frame', 'spec_bin'])
  ...
  if hparams.mix_weights_type:
    if hparams.mix_weights_type == 'pred_source':
      ...
      dense_config = network.update_config_from_kwargs(
          ...
          activation='linear')
      with tensorflow.compat.v1.variable_scope('mix_weights'):
...
#todo begin to train
  separated_waveforms = separate_waveforms(mixture_waveforms, hparams,is_graph)
  ...
  train_op = optimizer.minimize(
      ...
      global_step=tensorflow.compat.v1.train.get_or_create_global_step())
  # todo hanve changed
