...
def tdcn_block(x, config):
  ...
  with tf.variable_scope('tdcn_block'):
    ...
    # blstm = update_config_from_kwargs(config.blstm)
    # with tf.variable_scope('blstm'):
    #   dense1_config = update_config_from_kwargs(
    #         config.dense1, num_outputs=config.num_conv_channels,
    #         activation='linear')
    #   z = apply_blstm(input_tensor= z,output_name='output',params=blstm)
    # a = 1 # todo add to blstm
    #apply_blstm(input_tensor,output_name='output',params=256,config=None)
...
##############################################################
  # feature_map = tf.transpose(layer_activations, [0, 2, 3, 1])
  # feature_config = update_config_from_kwargs(config.initial_dense_layer, num_outputs=1250)
  # with tf.variable_scope('feature_transpose'):
  #     feature_out = dense_layer(feature_map, feature_config)
  # feature_out = tf.transpose(feature_out,[0,3,1,2])
  ...
  #     layer_activations = dense_layer(feature_out, initial_dense_config)
##############################################################
  initial_dense_config = update_config_from_kwargs(
      config.initial_dense_layer,
      num_outputs=config.prototype_block[0].bottleneck)
  ...
  input_of_block = []#定义了空白的block
  num_blocks = len(config.block_prototype_indices)#32
  ...
  with tf.variable_scope('improved_tdcn'):
    for block in range(num_blocks):
      ...
      input_of_block.append(layer_activations)#layer_activations:(1,none,1,256)
      scale_tdcn_block = find_scale_fn(block)#scale_tdcn_block 1.0
