...
def get_roomsim_spec(num_sources,
  ...
  spec[Features.RECEIVER_AUDIO] = tf.FixedLenFeature(
      [num_receivers, num_samples], tf.float32)
  #tf.FixedLenFeature返回一个定长detensor
...
def wavs_to_dataset(file_list,
  ...
  if not combine_by_class:
    ...
    for line in lines:
      ...
      if fixed_classes:
        # Find unique nonfixed classes in this line.默认none
  ...
  def decode_wav_or_return_zeros(wav):#todo
  ...
  dataset = dataset.map(decode_wav_or_return_zeros)#当前wav有值返回对应值，无值返回0
  ...
  # Build mixture and sources waveforms.todo 没看懂
...
def input_fn(params):
  ...
  if params.get('inference', False):
    ...
    with tf.variable_scope('input_audio'):
      ...
      #将key 和feature转换为placeholder的形式
  else:
    ...
    if not isinstance(file_list, list):
      file_list = read_lines_from_file(file_list, skip_fields=1,
                                       base_path=base_path)
      #将对应的file_list读成list的形式
    batch_size = params.get('batch_size', None)
    return wavs_to_dataset(file_list,
                           batch_size,
                           **io_params)
  #todo 调用wavs_to_dataset()
